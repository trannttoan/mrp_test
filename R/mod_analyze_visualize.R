      ...
      tabPanel("Individual Characteristics",
        tabsetPanel(
          ...
          tabPanel("County",
            tags$div(class = "pad_top",
               column(width = 9,
                      plotly::plotlyOutput(outputId = ns("indiv_county_map"),
                                           height = "500px")
               ),
               column(width = 3,
                      DT::dataTableOutput(outputId = ns("indiv_county_table"))
               )
            )
          )
        )
...
mod_analyze_visualize_server <- function(id, global){
  moduleServer( id, function(input, output, session){
    ...
    output$indiv_county_map <- plotly::renderPlotly({
      req(global$mrp_input)
    ...
    output$indiv_county_table <- DT::renderDataTable({
      req(global$mrp_input)

      global$mrp_input$brms_input |>
        group_by(fips) |>
        summarize(sample_size = sum(tests)) |>
        left_join(global$extdata$fips, by = "fips") |>
    ...
    output$prev_county <- plotly::renderPlotly({
      ...
      map_prev(global$mrp_input$brms_input, global$extdata$fips, global$plotdata$geojson)
    })
  })
}

## To be copied in the UI
# mod_analyze_visualize_ui("analyze_visualize_1")

## To be copied in the server
# mod_analyze_visualize_server("analyze_visualize_1")
